...
(define (rotate left right Acc) ;acc trebuie sa fie flux pentru ca il va inlocui pe left
...
(define (enqueue x q)
  ...
  (if (< (queue-size-l q) (+ (queue-size-r q) 1))
      (make-queue (rotate (queue-left q) (cons x (queue-right q)) empty-stream) '() (add1 (+ (queue-size-l q) (queue-size-r q))) 0)
...
(define (dequeue q)
  (if (< (- (queue-size-l q) 1) (queue-size-r q))
      (make-queue (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream) '() (+ (queue-size-l q) (queue-size-r q)) 0)
